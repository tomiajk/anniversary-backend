...
export async function deleteDecline(req: Request, res: Response) {
	try {
		const { declineId } = req.params;
		const decline = await Decline.findByIdAndDelete(declineId);

		if (decline)
			return res.status(200).json({ decline: "successfully deleted" });
		else
			return res.status(400).json({
				message: "decline with this id does not exist in the database",
			});
	} catch (error) {
		console.log("Error fetching this decline", error);
		return res.status(500).json({ message: "Error Occured", error });
